...
Redmine::Plugin.register :redmine_backlogs do
  ...
  menu :project_menu, :rb_kanban, { :controller => :rb_kanban, :action => :current }, :caption => :label_kanban, :after => :rb_taskboards, :param => :project_id, :if => Proc.new {|project| Backlogs.configured? && project && RbStory.product_backlog(project).size > 0}
